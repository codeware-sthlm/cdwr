...
const person = {
  fields: { elapsedMs: 10_000, honeypot: '', token: '' },
...
describe('guardSubmit', () => {
  ...
  it('leaves the counting alone when someone else already did it', async () => {
    const config = { limit: 1, windowMs: 60_000 };

    // Counted once by the app that saw the visitor's own address
    expect(
      await guardSubmit({ ...person, scope: 'form', rateLimit: config })
    ).toEqual({ ok: true });

    // Forwarded on, several times, and never counted again here — otherwise
    // one forwarding machine would spend a whole site's allowance
    for (let i = 0; i < 5; i++) {
      expect(
        await guardSubmit({ ...person, scope: 'form', forwarded: true })
      ).toEqual({ ok: true });
    }

    // The visitor's own bucket is untouched by any of that
    expect(
      await guardSubmit({ ...person, scope: 'form', rateLimit: config })
