...
export type GuardOptions = {
  ...
  secretKey?: string;
  /**
   * This request reached us through a first-party app that already counted the
   * visitor against its own allowance.
   *
   * The address on it is that app's machine, not the visitor's — so it is
   * neither counted here nor offered to Cloudflare as the address that solved
   * the challenge, which would be a different one and could fail a perfectly
   * good token.
   */
  forwarded?: boolean;
...
export async function guardSubmit({
  ...
  // Only when it is the visitor's own address. Through a first-party hop it is
  // the forwarding machine's, which answers no question worth asking
  const ip = forwarded ? undefined : clientIp(headers);
